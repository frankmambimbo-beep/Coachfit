...
class _PushupCameraScreenState extends ConsumerState<PushupCameraScreen> {
  ...
  // Only run pose detection on every 2nd frame — ML Kit inference
  // takes longer than the camera's native frame rate, so processing
  // every single frame just builds a backlog and makes tracking feel
  // laggy. Skipping frames keeps it responsive without losing accuracy,
  // since push-up reps happen over roughly a second, not milliseconds.
